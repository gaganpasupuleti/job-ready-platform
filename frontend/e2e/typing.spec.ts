...
test('full screen hides chrome and keeps the live session controls', async ({ page }) => {
  await openStudio(page)
  await page.getByRole('button', { name: 'Full screen' }).click()
  await expect(page.locator('.typing-studio')).toHaveAttribute('data-fullscreen', 'true')
  await expect(page.getByRole('heading', { name: 'Find your typing rhythm.' })).toHaveCount(0)
  await expect(page.getByRole('navigation', { name: 'Practice destinations' })).toHaveCount(0)
  ...
  await expect(page.getByText('FOCUS ON ACCURACY')).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Back', exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Restart typing session' })).toBeVisible()
  await expect(page.getByTestId('typing-wpm')).toBeVisible()
  await expect(page.getByTestId('typing-passage')).toBeVisible()
  await expect(page.locator('header.masthead')).not.toBeVisible()
  await expect(page.locator('.jr-breadcrumb')).not.toBeVisible()
})

test('full screen Back and Escape restore chrome without resetting the run', async ({ page }) => {
  await page.clock.install()
  await openStudio(page)
  await page.getByLabel('Session duration').selectOption('120')
  await page.getByLabel('Typing input', { exact: true }).focus()
  await page.keyboard.type('lea')
  await expect(page.getByLabel('Session duration')).toBeDisabled()
  const remainingBefore = await page.getByTestId('typing-time').innerText()
  await page.getByRole('button', { name: 'Full screen' }).click()
  await expect(page.locator('.typing-studio')).toHaveAttribute('data-fullscreen', 'true')
  await expect(page.getByLabel('Typing input', { exact: true })).toHaveValue('lea')
  await page.clock.fastForward(2000)
  await page.getByRole('button', { name: 'Back', exact: true }).click()
  await expect(page.locator('.typing-studio')).toHaveAttribute('data-fullscreen', 'false')
  await expect(page.getByRole('heading', { name: 'Find your typing rhythm.' })).toBeVisible()
  await expect(page.locator('header.masthead')).toBeVisible()
  await expect(page.getByLabel('Typing input', { exact: true })).toHaveValue('lea')
  await expect(page.getByLabel('Session duration')).toHaveValue('120')
  await expect(page.getByLabel('Session duration')).toBeDisabled()
  const remainingAfterBack = await page.getByTestId('typing-time').innerText()
  expect(Number(remainingAfterBack.replace('s', ''))).toBeLessThanOrEqual(
    Number(remainingBefore.replace('s', '')),
  )
  await page.getByRole('button', { name: 'Full screen' }).click()
  await page.getByLabel('Typing input', { exact: true }).focus()
  ...
  await expect(page.locator('header.masthead .new-brand')).toBeVisible()
})
